...
class WP_JSON_Server implements WP_JSON_ResponseHandler {
	...
	public function serve_request( $path = null ) {
		...
		$lang = $_GET['lang']; //peter modify
		$cookie = $_GET["cookie"]; //peter modify
		...
		if ( ! $served ) {
			...
			//<!--filter columns
			$result_temp = $this->prepare_response( $result );

			//<!--add filter columns
			$filter_columns = array('ID',
									'featured_image.guid',
									'featured_image.ID',
									'title',
									'date',
			...
			foreach ($filter_columns as $key => $value) {
				$tmp = explode('.', $value);
				if(!in_array($tmp[0], $filter1, true)){
			        array_push($filter1, $tmp[0]);
			    }
				
				if(count($tmp)==2){
					if(!in_array($value, $filter2, true)){
						array_push($filter2, $value);
					}
				}
			}

			$data_count = count($result_temp->data);
			if ($data_count == 0) {
				$is_array = is_array($result_temp);
				if ($is_array == true) {
					$is_assoc = $this->is_assoc($result_temp);
				}
			}else{
				$is_array = is_array($result_temp->data);
				$is_assoc = $this->is_assoc($result_temp->data);
			}
			

			if ($is_assoc == false && $is_array == true && $data_count != 0) {
				//assoc array
				//foreach($result_temp->data as $post){
				for ($i=0; $i < count($result_temp->data); $i++) { 

			        if(is_array($result_temp->data[$i])){

			        	foreach ($result_temp->data[$i] as $key1 => $value1) {
			        		//level1
		        			//判斷是否需要keep
		        			if(in_array($key1,$filter1)){
		        				// echo 'keep==>' . $key1 . '<br>';
		        			}else{
		        				// echo 'remove==>' . $key1 . '<br>';
		        				unset($result_temp->data[$i][$key1]);
		        			}

			        		if (is_array($value1)) {
			        			
			        			//search filter2, check是否需要處理,ex: author.*,如果有找到表示要filter
			        			$lvl2 = preg_grep('/^' . $key1 . '.*/',$filter2);
			        			if (count($lvl2)>0) {
			        				//要處理
			        				foreach ($value1 as $key2 => $value2) {
				        				if (in_array($key1 . '.' . $key2,$filter2)) {
				        					// echo 'keep==>' . $key1 . '.' . $key2 . '<br>';
				        				}else{
				        					// echo 'remove==>' . $key1 . '.' . $key2 . '<br>';
				        					unset($result_temp->data[$i][$key1][$key2]);
				        				}
				        			}
			        			} // end if (count($lvl2)>0)
			        	...
			        	} //end foreach $post as $key1 

			        } // end if(is_array($post))
			        
			    } //end for $result

			}else if ($is_assoc == true && $data_count != 0){
				//list array
				if(is_array($result_temp->data)){

		        	foreach ($result_temp->data as $key1 => $value1) {
		        		//level1
	        			//判斷是否需要keep
	        			if(in_array($key1,$filter1)){
	        				// echo 'keep==>' . $key1 . '<br>';
		        			...
		        			if (count($lvl2)>0) {
		        				...
		        				foreach ($value1 as $key2 => $value2) {
			        				if (in_array($key1 . '.' . $key2,$filter2)) {
			        					...
			        					// echo 'remove==>' . $key1 . '.' . $key2 . '<br>';
			        					unset($result_temp->data[$key1][$key2]);
			        				}
			        			}
		        			} // end if (count($lvl2)>0)
				...
				if(is_array($result_temp)){
		        	...
		        	foreach ($result_temp as $key1 => $value1) {
		        		//level1
	        			//判斷是否需要keep
	        			if(in_array($key1,$filter1)){
	        				// echo 'keep==>' . $key1 . '<br>';
