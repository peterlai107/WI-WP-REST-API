 ...
 * Version: 1.2.3
 * Author: Peter Lai
 * Plugin URI: https://github.com/peterlai107/WI-WP-REST-API
 *
 * This program is base on "https://github.com/WP-API/WP-API" v1.2.3,
 * do some modify for special purpose.
 *
...
function json_cookie_check_errors( $result ) {
	...
	// Check the nonce.
//echo ('verify nonce...' . $nonce . '<br>');
//echo print_r(wp_get_current_user());
...
//print_r($post_type);
	...
	switch ( $capability ) {
		...
		case 'edit' :
			if ( current_user_can( $post_type->cap->edit_post, $post['ID'] ) || current_user_can( 'edit_reply', $post['ID'] ) || current_user_can( 'edit_topic', $post['ID'] ) ) {
                    		$permission = true;
               		}
			break;
...
//echo 'info ' . $post_type->cap->create_posts . '<br>';
//echo 'create_posts ' . current_user_can( $post_type->cap->create_posts ) . '<br>';
//echo 'edit_posts ' . current_user_can( $post_type->cap->edit_posts ) . '<br>';
//echo 'read_posts ' . current_user_can( $post_type->cap->read_post ) . '<br>';
//echo 'publish_topics ' . current_user_can( 'publish_topics' ) . '<br>';
//echo 'read_hidden_forums ' . current_user_can( 'read_hidden_forums' ) . '<br>';




			if ( current_user_can( $post_type->cap->create_posts ) || current_user_can( $post_type->cap->edit_posts ) || current_user_can( 'publish_topics' ) || current_user_can( 'publish_replies' ) ) {
                    		$permission = true;
               		}
			break;

		case 'delete' :
			if ( current_user_can( $post_type->cap->delete_post, $post['ID'] ) || current_user_can( 'delete_reply', $post['ID'] ) || current_user_can( 'delete_topic', $post['ID'] ) ) {
                    		$permission = true;
               		}
			break;
